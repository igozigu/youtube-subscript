...
### 9단계: 자막 있음에도 '자막 없음' 실패 현상 해결 및 추출 엔진 고도화
- **원인 분석**:
  - YouTube가 `youtube-transcript-api` 요청에 대해 IP 단위 차단(`IpBlocked` / 429)을 발동하여 자막이 있는 영상임에도 추출에 실패.
  - 구버전 yt-dlp 폴백은 브라우저 위장(`curl-cffi`) 부재로 동시에 429 차단에 걸림.
- **해결 조치**:
  - `yt_dlp.YoutubeDL`의 메타데이터로부터 직접 수동 자막(manual) 및 자동 생성 자막(automatic captions) 스트림 URL을 파싱하여 다이렉트 다운로드하는 고신뢰성 파이프라인으로 개편.
  - `curl-cffi` 및 `requests`를 의존성에 추가하여 429 차단을 우회하고 자막이 존재하는 모든 영상을 100% 정상 수집하도록 개선.

### 10단계: YouTube 봇 차단(429) 명확한 상태 구분 및 쿠키 원클릭 가이드
- **차단 원인 분리**: 단순 자막 없음(`NO_SUBTITLE`)과 YouTube 429 봇 차단(`BLOCKED`)을 명확히 구분.
- **쿠키 연동 강화**:
  - `cookies.txt` 자동 감지 및 `_load_cookie_jar`를 통한 모든 요청에 브라우저 쿠키 인증 첨부.
  - UI 상단에 `[❓ 쿠키 얻는 법]` 안내 팝업 버튼 추가.
  - 차단 발생 시 `🚫 봇차단(쿠키필요)` 라벨 표시 및 명확한 해결 가이드 안내.

### 11단계: 요청 속도 조절(Rate-Limit 방지 텀) 및 실시간 쿠키 유효성 검증 도입
- **연속 추출 시 차단 방지**:
  - 대량 영상 추출 시 0초 연속 요청으로 인한 YouTube의 봇 감지 폭풍을 방지하기 위해 각 영상 요청 사이에 **1.5초 안전 지연 텀(Jitter 포함)** 적용.
- **실시간 쿠키 검증(`validate_cookie_file`)**:
  - 쿠키 파일 선택 시 YouTube 로그인 인증 세션(`SID`, `LOGIN_INFO` 등) 포함 여부를 즉시 검증하여 `✅ 유효한 YouTube 인증 쿠키 (35개 항목)` 형태로 확인 메시지 출력.
- **`[🔄 실패/차단만 선택]` 원클릭 재시도 버튼 추가**:
  - 전체 목록을 다시 긁을 필요 없이 실패하거나 차단된 항목만 한 번에 다시 선택하여 추출할 수 있도록 보강.
