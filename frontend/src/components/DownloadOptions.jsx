...
function DownloadOptions({ jobData, onReset }) {
  ...
  useEffect(() => {
    getJobStatus(jobId).then(status => {
      ...
      // 백엔드 필드: results (not items), status는 VideoStatus enum 값
  ...
  return (
    <div className="card">
      ...
      <div style={{marginBottom: '2rem', padding: '1rem', backgroundColor: '#f9f9f9', borderRadius: '4px'}}>
        ...
        <div style={{display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem'}}>
          <span>⚠️ 자막 없음:</span> <strong style={{color: 'var(--warning)'}}>{stats.noSubtitle}개</strong>
        </div>
      ...
      <div className="download-buttons">
        <button className="download-btn" onClick={() => handleDownload('md')}>
